...
function ContactPage() {
  ...
  return (
    <div className={cl.contactPage} id="contacts">
      <div className={cl.container}>
        <div>
          <h2>Ждем Вас</h2>
          <hr className='blk'/>
        </div>
        <div onClick={() => setInfoMessage({...infoMessage, message: ''})} className={infoClass.join(' ')}>{infoMessage.message}</div>
        <div className={cl.col2}>
          <InfoBar/>
          <div onClick={() => setInfoMessage({...infoMessage, message: ''})} className={infoMinClass.join(' ')}>{infoMessage.message}</div>
